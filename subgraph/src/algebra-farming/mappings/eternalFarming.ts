...
import { Deposit, Mint, Reward, EternalFarming, Pool, Token } from '../../../generated/schema';
...
export function handleIncentiveCreated(event: EternalFarmingCreated): void {
  ...
  if (entity == null) {
    ...
    entity.amount0 = BigDecimal.fromString("0");
    entity.amount1 = BigDecimal.fromString("0");
...
export function handleTokenStaked(event: FarmEntered): void {
  ...
  if (entity != null) {
    ...
    if (eternalFarming) {
      ...
      eternalFarming.totalAmountUSDEstimated = eternalFarming.amount0.times(token0!.derivedMatic).plus(eternalFarming.amount1.times(token1!.derivedMatic))
