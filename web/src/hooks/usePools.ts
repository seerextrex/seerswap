...
export function usePools(poolKeys: [Currency | undefined, Currency | undefined][], listenerOptions?: ListenerOptions): [PoolState, Pool | null][] {
    ...
    return useMemo(() => {
        return poolKeys.map((_key, index) => {
            ...
            if (!globalState.price || globalState.price === 0n) return [PoolState.NOT_EXISTS, null];

            try {
                // Convert bigint values to appropriate types for Pool constructor
                ...
                return [PoolState.EXISTS, new Pool(
                    token0,
                    token1,
                    feeAsNumber,
                    priceAsString,
                ...
                console.error(`Error constructing pool for ${token0.symbol}/${token1.symbol}:`, error);
