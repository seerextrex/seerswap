...
export const EnhancedPool = ({ token0, token1, market0, market1, fee, address }: PoolData) => {
    ...
    // Get outcome names
    const token0DisplayName = useMemo(() => {
        if (market0) {
            const outcomeName = getOutcomeName(market0 as any, token0.id);
            return outcomeName || token0.symbol;
        } else if (market1 && !market0) {
            // If only market1 exists, token0 might be the collateral
            return token0.symbol;
        }
        return token0.symbol;
    }, [token0, market0, market1]);
    
    const token1DisplayName = useMemo(() => {
        if (market1) {
            const outcomeName = getOutcomeName(market1 as any, token1.id);
            return outcomeName || token1.symbol;
        } else if (market0 && !market1) {
            // If only market0 exists, token1 might be the collateral
            return token1.symbol;
        }
        return token1.symbol;
    }, [token1, market0, market1]);
    ...
    }, [token0, token1, token0DisplayName, token1DisplayName]);
